...
btn5.addEventListener('click',changeYellow)
function changeYellow(){
    let pEle = document.getElementsByClassName('yellow');
    let children = pEle.children;
    let pvalue = pEle[0].textContent;
    let preplace = pvalue.replace(/,/gi, '');
    let psplit = preplace.split(" ");
    let newspan = document.createElement("span");
    console.log(children);
    newspan.style.backgroundColor = 'white'
    for (let i = 0; i<psplit.length;i++){
        if(psplit[i].length==5){
            pEle = document.body.appendChild(newspan);
        }
        
    }
    newspan.style.backgroundColor = 'yellow';
    console.log(newspan.style.backgroundColor)
}

let inputId = querySelector('inputid');
